...
const watch = function () {
    ...
    server.watch(["../appl/**/*.js", "../appl/**/*.ts"]).on("change", (file) => {
...
let stripCodePlugin = {
    ...
    setup(build) {
        ...
        build.onLoad({ filter: /^.*esbuild.*\.(js|ts)$/ }, async (args) => {
...
function copyCss() {
    src(["../appl/view*/**/*", "../appl/css/**/*"])
        .pipe(copy("../../" + dist, { prefix: 2 }));
    return src(["../appl/css/**/*.css",])
        .pipe(flatten({ includeParents: -2 })
            .pipe(dest("../../" + dist + "/appl/css")));
}
