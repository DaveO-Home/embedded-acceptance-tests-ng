...
export class ToolsSelect {
    ...
    getMessage() {
        ...
        let index = -1
        items.forEach((item: { displayed: any; }, _index: number) => {
            if (item.displayed) {
                index = _index
            }
        })
    ...
    onCompletedClick(e: { preventDefault: () => void; target: { text: string; }; }) {
        let dropdownValue;
        e.preventDefault();
...
export class ToolsComponent implements OnInit {
    ...
    ngOnInit(): Promise<void> {
        return this.tables.getHtml(this).then(function (data: { response: string | Element | Comment | DocumentFragment | ((this: HTMLElement, index: number, oldhtml: string) => string | Element | Text | Comment | DocumentFragment); }) {
			/*
