...
export default {
    ...
    toViewsUrl (url) {
        return _startsWith(url, "views/") ? this.toScriptsUrl(url) : this.toUrl(url);
    },
    loadController (controllerName, controller, fnLoad, fnError) {
        ...
        if (this.controllers[controllerName]) {
            ...
            try {
                ...
                /* develblock:end */
                me.controllers[_capitalize(controllerName)] = appController;
    ...
    loadView (options, fnLoad) {
        if (options && fnLoad) {
            ...
            const currentController = this.controllers[_capitalize(options.controller)];
